/**
 * Auth token handling shared by the apps' AuthGuards.
 *
 * The token is issued by the portfolio's login page and stored in localStorage
 * under a key every app on the domain shares. The client only decodes it to
 * decide what to render; the PHP API verifies it on every request.
 */
