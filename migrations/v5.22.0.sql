...
INSERT INTO changelog_changes (id, release_id, type, text, sort_order) VALUES
  ...
  (UUID(), @rid, 'changed', 'Every place that reports how many cards a deck holds now runs through one shared counter, replacing sixteen separate copies of the same calculation. The deck page, the type breakdown, the deck legality check, and the AI coach can no longer disagree about the size of the same deck.', 4);
